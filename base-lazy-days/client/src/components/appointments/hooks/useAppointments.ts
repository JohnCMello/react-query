...
export function useAppointments(): UseAppointments {
  ...
  useEffect(() => {
    const nextMontYear = getNewMonthYear(monthYear, 1);
    queryClient.prefetchQuery(
      [queryKeys.appointments, nextMontYear.year, nextMontYear.month],
      () => getAppointments(nextMontYear.year, nextMontYear.month),
    );
